...
export function ChatComponent() {
  ...
  const handleSubmit = async (e?: React.FormEvent, overrideInput?: string) => {
    ...
    try {
      ...
      const newDynamicPrompts = responseData.recommendedQuestions || [];
      ...
      setDynamicPrompts(newDynamicPrompts);

      console.log('Received assistant response:', assistantResponse);
      console.log('New dynamic prompts:', newDynamicPrompts);
    } catch (error) {
  ...
  const recommendedPrompts = [
    { text: "Summary of the big poo", prompt: "Can you provide a summary of the podcast?" },
    { text: "The big ideas", prompt: "What are the main big ideas discussed in this podcast?" },
    { text: "Fringe ideas", prompt: "Are there any fringe or unconventional ideas mentioned in the podcast?" },
  ];
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-2">
            {dynamicPrompts.length > 0 ? (
              ...
              recommendedPrompts.map((prompt, index) => (
                <Button
                  key={index}
                  onClick={() => handleSubmit(undefined, prompt.prompt)}
                  className="bg-[#161B22] text-[#21C55D] hover:bg-[#21C55D] hover:text-white border border-[#21C55D] transition-colors duration-200 rounded-lg"
                >
                  {prompt.text}
                </Button>
